...
public class Solution {
    ...
    public Node copyRandomList(Node head) {
        ...
        // Now we need to add the pointers for next and random
        // to the copied Nodes we added to our dictionary
        Node curr = head;
        while(curr != null) {
            Node copy = oldToNew[curr];

            // Assign next node using the dict key
            // Using a ternary since some vals can be null (end nodes)
            copy.next = curr.next != null ? oldToNew[curr.next] : null;
            // Assign random pointer using dict
            copy.random = curr.random != null ? oldToNew[curr.random] : null;
