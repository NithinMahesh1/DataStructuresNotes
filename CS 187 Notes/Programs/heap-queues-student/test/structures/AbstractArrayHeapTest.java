...
public class AbstractArrayHeapTest {
  ...
  private AbstractArrayHeap<Integer, String> friendHeap;

  @Before
  public void setup() {
    arrayHeap = new StudentArrayHeap<String, Integer>(new StringLengthComparator());
    friendHeap = new StudentArrayHeap<Integer, String>(new ReverseIntegerComparator());
  }
  ...
  @Test (timeout = 100)
  public void myTest() {
	  assertEquals(0, friendHeap.size());
	  friendHeap.add(1, "Nick");
	  friendHeap.add(2, "Dave");
	  
	  assertEquals(2, friendHeap.size());
	  assertEquals("Nick", friendHeap.remove());
	  assertEquals(1, friendHeap.size());
  }
